...
const CommentItem = ({ comment,userId }: CommentItemProps) => {
  ...
  const commentDeleteHandeler = async()=>{
    try {
      if(confirm("you want delete this message , are you sure")){
  ...
  return (
    <div className="mb-5 rounded-lg p-3 bg-gray-200 border-2 border-gray-300">
      ...
      {userId && comment.userId === userId && (
              ...
              />
              <FaTrash onClick={commentDeleteHandeler } className="text-red-600 text-xl cursor-pointer" />
            </div>
      )}
      {open && (
        <UpdateCommentModal
